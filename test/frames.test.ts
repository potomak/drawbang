...
import { cloneFrame, newFrame, type Frame } from "../src/editor/layers.js";
import { History } from "../src/editor/history.js";

const MAX = 16;

function freshState(): FrameState {
  return newFrameState(16, 16);
}

// Test helper: edit the current frame's first layer in-place. Since the
// existing assertions read pixel (0, 0) on the single layer, this keeps
// every test focused on the frame-level invariants rather than layer
// bookkeeping.
function paint(state: FrameState, value: number): () => void {
  const idx = state.current;
  const before = cloneFrame(state.frames[idx]);
  state.frames[idx].bitmaps[0].set(0, 0, value);
  return () => {
...
function pixelAt(f: Frame, x: number, y: number): number {
  return f.bitmaps[0].get(x, y);
}
...
test("addFrame returns null at the frame cap", () => {
  const state: FrameState = {
    layers: [{ id: "x", name: "Layer 1", visible: true }],
    frames: Array.from({ length: MAX }, () => newFrame(16, 16, 1)),
    current: 0,
    currentLayer: 0,
  };
  const undo = addFrame(state, MAX);
...
test("removeCurrentFrame undo restores frame at the original index", () => {
  const state = freshState();
  addFrame(state, MAX);
  state.frames[1].bitmaps[0].set(3, 3, 5);
  state.current = 1;
  const undo = removeCurrentFrame(state);
  assert.equal(state.frames.length, 1);
  assert.equal(state.current, 0);
  ...
  assert.equal(state.frames.length, 2);
  assert.equal(state.current, 1);
  assert.equal(state.frames[1].bitmaps[0].get(3, 3), 5);
});

test("pasteIntoCurrent replaces the current frame and undo restores it", () => {
  const state = freshState();
  state.frames[0].bitmaps[0].set(0, 0, 1);
  const clip: Frame = { bitmaps: [new Bitmap(16, 16)] };
  clip.bitmaps[0].set(15, 15, 7);
  const undo = pasteIntoCurrent(state, clip);
  assert.equal(pixelAt(state.frames[0], 15, 15), 7);
  assert.equal(pixelAt(state.frames[0], 0, 0), TRANSPARENT);
  undo();
  assert.equal(pixelAt(state.frames[0], 0, 0), 1);
  assert.equal(pixelAt(state.frames[0], 15, 15), TRANSPARENT);
});

test("pasteIntoCurrent stores a clone so later edits do not leak", () => {
  const state = freshState();
  const clip: Frame = { bitmaps: [new Bitmap(16, 16)] };
  clip.bitmaps[0].set(1, 1, 2);
  pasteIntoCurrent(state, clip);
  clip.bitmaps[0].set(1, 1, 9);
  assert.equal(pixelAt(state.frames[0], 1, 1), 2);
});
...
test("undo stack: add → stroke → stroke → delete → undo restores the deleted frame", () => {
  ...
  assert.equal(state.frames.length, 2);
  assert.equal(state.current, 1);
  assert.equal(pixelAt(state.frames[1], 0, 0), 4);
});
...
test("undo stack: fully reverses add → stroke → stroke → delete in order", () => {
  ...
  assert.equal(pixelAt(state.frames[1], 0, 0), 3);

  history.undo(); // un-stroke-1
  assert.equal(pixelAt(state.frames[1], 0, 0), TRANSPARENT);
...
test("undo stack: stroke on non-current frame restores that frame, not the viewed one", () => {
  ...
  assert.equal(pixelAt(state.frames[0], 0, 0), TRANSPARENT);
  assert.equal(pixelAt(state.frames[1], 0, 0), 7);
